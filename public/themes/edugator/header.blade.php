...
@yield('page-css')
    ...
    <link rel="stylesheet" href="{{theme_asset('css/style.css')}}">
	 ...
	 @php if(get_option('enable_rtl')) { @endphp
...
<div class="main-navbar-wrap">
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-white">
		...
		<button type="button" class="navbar-toggle new-nav-toggle-btn" data-toggle="collapse" data-target="#bs-sidebar-navbar-collapse-1">
            ...
            <div class="collapse navbar-collapse" id="mainNavbarContent">
                <ul class="navbar-nav categories-nav-item-wrapper">
                    <li class="nav-item nav-categories-item">
                        <a class="nav-link browse-categories-nav-link bg-white nav-cat-btn" href="{{route('categories')}}"> <i class="la la-th-large"></i> {{__t('categories')}}</a>
                ...
                <div class="header-search-wrap ml-2">
                    <form action="{{route('courses')}}" class="form-inline " method="get">
                        <input class="form-control" type="search" name="q" value="{{request('q')}}" placeholder="ابحث">
                ...
                <ul class="navbar-nav main-nav-auth-profile-wrap">
                    ...
                    @if (Auth::guest())
                        <li class="nav-item mr-2 ml-2">
                            <a class="nav-link btn btn-login-outline no-border" href="{{route('login')}}"> {{__t('login')}}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link btn btn-theme-primary nav-reg-btn" href="{{route('register')}}"> {{__t('signup')}}</a>
                        </li>
                    @else
                        ...
                        <li class="nav-item main-nav-right-menu nav-item-user-profile">
                            ...
                            <div class="profile-dropdown-menu pt-0">
                                ...
                                <div class="profile-dropdown-userinfo bg-light p-3">
                                    <p class="m-0 header-nav-name">{{ $auth_user->name }}</p>
                                    <small>{{$auth_user->email}}</small>
                                </div>

                                @include(theme('dashboard.sidebar-menu-top'))
                            </div>
                        </li>
                    @endif
