                ...
                <div class="quiz-instruction-wrapper">
                    ...
                    @if(! $attempt || $attempt->status !== 'finished')
                        <div class="assignment-header-info mb-4 p-3">
                            <h4 class="mb-4">إرشادات</h4>

                            <p class="assignment-basic-info mb-2">
                                <label class="ml-4"> <i class="la la-clock"></i> <strong>المدة الزمنية:  </strong> {{$quiz->option('time_limit')}} دقيقة </label>

                                <label class="ml-4"><i class="la la-chart-pie"></i> <strong>عدد الأسئلة:  </strong> {{$quiz->option('questions_limit')}}</label>
								
                                <label class="ml-4"><i class="la la-check-circle"></i> <strong>الحد الأدنى من الدرجات للنجاح:</strong> {{$quiz->option('passing_score')}}%</label>
                            </p>

                            <p class="font-italic text-muted m-0">
                                يجب أن تنهي الاختبار في جلسة واحدة، حيث لن يسمح لك بحفظ الإجابات والمتابعة في وقت لاحق، لن تتمكن من تخطي السؤال دون الإجابة عليه.
                            </p>
                        </div>
                    @endif
                    ...
                    @if($attempt)
                        ...
                        @if($attempt->status === 'in_review')
                            <div class="bg-light-sky p-4 border d-flex quiz-submitted-alert">
                                <h1 style="font-size:24px;" class="ml-3">
                                    <i class="la la-info-circle"></i>
                                </h1>
                                <div>
                                    <h3 style="font-size:18px;">تم إرسال إجاباتك بنجاح</h3>
                                    <p>سيتم مراجعة إجاباتك من قبل المدرب، وسترى نتيجتك قريباً في لوحتك التعليمية</p>
                                </div>
                            </div>
                        @endif
