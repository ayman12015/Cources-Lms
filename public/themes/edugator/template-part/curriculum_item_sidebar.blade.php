...
<div class="lecture-sidebar">
    ...
    <h4 class="p-4 lecture-sidebar-course-title">{{$course->title}}</h4>
                            ...
                            @foreach($section->items as $item)
                                ...
                                <div class="sidebar-section-item {{$item->id == $content->id? 'active' : ''}} {{$is_completed? 'completed' : ''}}">
                                    <div class="section-item-title">
                                        ...
                                        <a href="{{route('single_'.$item->item_type, [$course->slug, $item->id ] )}}" class="course-nav-item-left p-2 d-flex" @if($is_completed) data-toggle="tooltip" title="{{__t('completed')}}" @endif>
                                            <span class="lecture-status-icon border-right pr-1">
                                                @if($is_completed)
                                                    <i class="la la-check-circle text-success"></i>
                                                @else
                                                    <i style="display:none;" class="la la-circle"></i>
                                                @endif
                                            </span>
                                            <div class="title-container pr-2 flex-grow-1 d-flex">
                                                <span class="lecture-icon pl-1"> {!! $item->icon_html !!}</span>
