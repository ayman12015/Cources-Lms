        ...
        <div class="row">
            ...
            <div class="col-md-5">
                <div class="input-group">
                    <select name="status" class="ml-3">
                        ...
                        <option value="dispute">مخالف</option>
                        <option value="expired">منتهي</option>
                    </select>

                    <button type="submit" name="bulk_action_btn" value="update_status" class="btn btn-primary ml-2">{{__a('update')}}</button>
            ...
            <div class="col-md-7">
                ...
                <div class="search-filter-form-wrap mb-3">
                    <div class="input-group">
                        <input type="text" class="form-control mr-3" name="q" value="{{request('q')}}" placeholder="فلترة بالاسم و البريد الإلكتروني">
                        <select name="filter_status" class="form-control mr-3">
                            <option value="">فلترة بالحالة</option>
                            <option value="initial" {{selected('initial', request('filter_status'))}} >مبدئي</option>
                            <option value="pending" {{selected('pending', request('filter_status'))}} >في الإنتظار</option>
                            <option value="onhold" {{selected('onhold', request('filter_status'))}} >معلقة</option>
                            <option value="success" {{selected('success', request('filter_status'))}} >ناجح</option>
                            <option value="failed" {{selected('failed', request('filter_status'))}} >فاشل</option>
                            <option value="declined" {{selected('declined', request('filter_status'))}} >مرفوض</option>
                            <option value="dispute" {{selected('dispute', request('filter_status'))}} >مخالف</option>
                            <option value="expired" {{selected('expired', request('filter_status'))}} >منتهي</option>
                        </select>

                        <button type="submit" class="btn btn-primary btn-purple mr-3"><i class="la la-search-plus"></i>فلترة</button>
                    </div>
                </div>
            </div>
